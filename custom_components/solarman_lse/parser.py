...
#ADNOVEA FIX start
            ...
            self.try_parse_hour(rawData,definition, start, length)
#ADNOVEA FIX end
        return
    ...
    def try_parse_signed (self, rawData, definition, start, length):
        ...
        for r in definition['registers']:
            ...
            if (index >= 0) and (index < length):
                maxint <<= 16
                maxint |= 0xFFFF
                offset = OFFSET_PARAMS_LSE + (index * 2)
    ...
    def try_parse_datetime (self, rawData, definition, start, length):
        ...
        for i,r in enumerate(definition['registers']):
            ...
            if (index >= 0) and (index < length):
                ...
                temp = struct.unpack('>H', rawData[offset:offset + 2])[0]
                if(i==0):
                    value = value + str(temp >> 8)   + "/"
                    if int(temp & 0xFF) < 10:
                        value = value + "0"
                    value = value + str(temp & 0xFF) + "/"
                elif (i==1):
                    if int(temp >> 8) < 10:
                        value = value + "0"
                    value = value + str(temp >> 8)   + " "
...
#ADNOVEA FIX start
    def try_parse_hour (self, rawData, definition, start, length):
        title = definition['name']
        value = 0
        found = True
        shift = 0
        ...
        if found:
            ...
            if len(st_num) == 1:
                ...
                st_num = "0" + st_num
            self.result[title] = st_num[0:2] + ':' + st_num[2:4]
        return  
#ADNOVEA FIX end
